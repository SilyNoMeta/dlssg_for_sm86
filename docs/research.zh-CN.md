...
## 310.9.1-11：提前加载引擎与可选面板

DLL 将导入拦截限制在游戏程序、本地加载模块和已识别 NVIDIA 路径。GetProcAddress 保留 Windows 加载函数的实际身份，避免覆盖层把桥接包装函数保存为原函数而形成递归。

覆盖层仍可能改写导入表并使用已保存的 Windows 地址。因此在 DllMain 外运行一次的工作线程通过 MinHook 拦截五个加载/解析 API，内部调用使用原函数跳板。保留早期导入拦截，因为线程调度不能保证先于所有初始化。NvAPI 路由区分公共入口和驱动内部实现，仅对 DLSSG 调用方进行替换。测试覆盖反复模拟覆盖层接管、真实 NvAPI 初始化及原生 DX12/Vulkan 控制请求，不代表实际呈现帧的性能测试。

ReShade add-on 是控制客户端，不包含第二个引擎。GPU 模型、权重、时序修复及四项优化与 -10 相同。启动和隔离控制测试并不代表新的性能或画质验证。

实时固定倍率和动态控制现在也支持只提交选项、不查询能力的兼容 DX12 集成。当明确的覆盖请求需要能力信息时，桥接层进行次数受限的查询。该查询消耗的呈现帧计数会在游戏下一次成功查询状态时仅补回一次；不会重放各视口的同步 fence 或状态。日志仅记录有限次数的模式与结果变化，帮助区分命令被接受和实际生成帧。
...
[DLSS render scale](super-resolution.zh-CN.md) · [Release notes](../RELEASE-NOTES.md)
