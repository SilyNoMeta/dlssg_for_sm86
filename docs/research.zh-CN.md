...
[Issue #2](https://github.com/SilyNoMeta/dlssg_for_sm86/issues/2) 报告 RTX 3060 Ti 8 GB 在赛博朋克和鬼武者中 X4
...
赛博朋克、鬼武者/ASI loader 和修正版 Vulkan 游戏测试仍待确认。
...
悟空 1.0.21.23831，DX12，2560×1440，高画质，全景光线追踪低、超分辨率 58、
强运动模糊；RTX 3070 Ti **Laptop 8 GB**、i9-12900H、驱动 616.92。
用户报告 NR/Cost Scaler 设置与捕获起点保持一致；CSV 未记录 NR 的准确设置值。
每种条件仅一次约 60 秒捕获。

| X4 配置 | 显示 FPS | 1% low FPS | 平均 PC 延迟 |
|---|---:|---:|---:|
| 原生 310.1 | 106.151 | 61.112 | 87.691 ms |
| 旧 -4 | 103.710 | 72.330 | 88.099 ms |
| 修正 DLL（-7） | 98.064 | 66.295 | 91.377 ms |

**修正版本测试期间，Codex 伴侣窗口可见但未使用，与此前条件不同。**
因此这些数字不能证明修正导致 FPS 回退或提升。关键新证据是用户确认 X4 明显更流畅，
与独立运动测试一致。完整游戏基准截图显示 102 FPS，测量区间不同，不能替代
60 秒 FrameView 的 98.064 FPS。

[捕获汇总](benchmark-summary.csv) 包含最初 -0…-4 与后续原版/修正版。
源文件哈希用于识别数据，不发布个人原始日志。FPS=1000N/ΣΔt，使用
MsBetweenDisplayChange。1%/0.1% low 为最长 ceil(0.01N)/ceil(0.001N) 个间隔的
平均值取倒数。保留零间隔与尖峰；PC 延迟为 MsPCLatency 平均值。
一轮中的帧不是独立重复实验，因此不以数千帧构造误导性的置信结论。
欢迎其他 GPU/游戏的反馈，同时评估运动与响应感受。
