# DLSSG — 独立 ReShade add-on（310.9.1-10）

安装 **支持完整 add-on 的 ReShade + DLSSG.addon64 + dlssg_sm86.ini**。
兼容性引擎已集成到 add-on，无需本项目单独的 DLL/ASI。INI 放在 add-on 旁。
重启前移除本项目旧的 version.dll / dxgi.dll / ASI 和 DLSSGControls.addon64。
保留 ReShade 本身、必要的其他加载器和 NR add-on；仅移除本项目旧文件。

ReShade 必须在 DLSSG 初始化前加载本 add-on，部分游戏可能太晚加载。
为避免 ReShade 重建图形设备后留下失效的函数指针，引擎保留到进程退出。
停用或移除引擎需要重新启动游戏。不要同时使用独立的 Ada MFG 解锁 add-on。
